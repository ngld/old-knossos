...
Section
    ...
    WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\Knossos" "DisplayIcon" "$\"$INSTDIR\fs2mod-py.exe$\",0"
    ...
    CreateShortcut "$SMPROGRAMS\$StartMenuFolder\Knossos.lnk" "$INSTDIR\fs2mod-py.exe"
    CreateShortcut "$SMPROGRAMS\$StartMenuFolder\Launch FSO.lnk" "$INSTDIR\fs2mod-py.exe" "fso://run"
...
Section "fso:// Support"
    ExecWait '"$INSTDIR\fs2mod-py.exe" --install-scheme --silent'
SectionEnd
